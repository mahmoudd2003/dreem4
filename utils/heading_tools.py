...
# -*- coding: utf-8 -*-
"""
Heading enforcement utilities:
- enforce_outline(text, required_h2, required_h3_map): ensure required H2/H3 headings exist.
- default_required(symbol): default required headings for dream-interpretation articles.
"""
...
def enforce_outline(text: str, required_h2: List[str], required_h3_map: Dict[str, List[str]]) -> str:
    """
    Ensure the outline contains all required top-level (H2) and sub (H3) headings.
    - If a required H2 is missing, append it at the end with a placeholder bullet.
    - If a required H3 under a present H2 is missing, append it directly after that H2.
    Returns the amended outline text.
    """
    text = text or ""
    ...
    # Append missing H3 under their parent if parent exists
        ...
        while i < len(parts):
            if i + 1 < len(parts) and parts[i+1].lstrip().startswith("## "):
                ...
                if h2_line.strip()[3:] == parent:
                    # inject missing H3 at start of this section
                    inject = "".join([f"### {sub}\n- (Placeholder) نقطة موجزة.\n" for sub in missing_subs])
                    section = inject + section
                rebuilt.extend([pre, h2_line, section])
                i += 3
            else:
...
def default_required(symbol: str = "") -> Tuple[List[str], Dict[str, List[str]]]:
    ...
    required_h3_map = {
        "حالة الرائي": ["عزباء", "متزوجة", "حامل", "مطلقة", "رجل"],
    }

    s = (symbol or "").strip()
    if s in ("المال", "مال", "النقود", "النقد"):
        required_h3_map["الحالات المؤثرة"] = [
            "مال ورقي vs معدني",
            "العثور",
            "الضياع",
            "السرقة",
